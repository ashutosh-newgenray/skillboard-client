...
function generateRows() {
  const data = [];
  skills.map((skill) => {
    let group = skillGroups.filter((sg) => sg.id == skill.skill_group);
    data.push({
      ...skill,
      section: group[0].section,
      skill_group: group[0].skill_group,
    });
  });
  return data;
}
...
const headCells = [
  { id: "skill_group", label: "Skill Group" },
...
export default function Skill() {
  ...
  const [orderBy, setOrderBy] = React.useState("section");
